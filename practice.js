...
var getUserTweets = function(tweets, user) {
  ...
  _.each(tweets, function (tweet) {
    if (tweet.user === user) { out.push(tweet); }
  });
  return out;
};
...
var applyCoupon = function (groceries, coupon) {
  return _.map(groceries, function(item) {
    ...
    item.salePrice = '$' + salePrice.toString();
    return item;
  });
};
...
var dessertCategories = function (desserts) {
  return _.reduce(desserts, function (output, item) {
...
var countMessagesPerUser = function(tweets) {
  return _.reduce(tweets, function (output, item) {
    if (output[item.user] === undefined) {
      output[item.user] = 1;
    } else {
      output[item.user]++;
    }
